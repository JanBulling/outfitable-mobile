...
class Mesh {
  Mesh(
    ...
    this.textureRect =
        textureRect ?? Rect.fromLTWH(0, 0, texture?.width.toDouble() ?? 1.0, texture?.height.toDouble() ?? 1.0);
  }
...
Future<List<Mesh>> _buildMesh(
  ...
  for (int index = 0; index < elementOffsets.length; index++) {
    ...
    if (imageEntry != null) {
      _remapZeroAreaUVs(
          newTexcoords, newTextureIndices, imageEntry.value.width.toDouble(), imageEntry.value.height.toDouble());
    }
...
void _remapZeroAreaUVs(
    List<Offset> texcoords, List<Polygon> textureIndices, double textureWidth, double textureHeight) {
...
void _rebuildVertices(
    List<Vector3> vertices, List<Offset> texcoords, List<Polygon> vertexIndices, List<Polygon> textureIndices) {
