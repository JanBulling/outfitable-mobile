...
typedef ModelCreatedCallback = void Function(Model model);

class Scene {
  Scene({VoidCallback? onUpdate, ModelCreatedCallback? onModelCreated}) {
    this._onUpdate = onUpdate;
    this._onModelCreated = onModelCreated;
    world = Model(scene: this);
  }
  ...
  ModelCreatedCallback? _onModelCreated;
  int vertexCount = 0;
  int faceCount = 0;
  bool _needsUpdateTexture = false;

  // calculate the total number of vertices and faces
  void _calculateVertices(Model m) {
    vertexCount += m.mesh.vertices.length;
    faceCount += m.mesh.indices.length;
    final List<Model> children = m.children;
  ...
  void _renderModel(RenderMesh renderMesh, Model m, Matrix4 model, Matrix4 view, Matrix4 projection) {
    if (!m.visiable) return;
    model *= m.transform;
    ...
    final List<Vector3> vertices = m.mesh.vertices;
    ...
    if (m.lighting) {
      ...
      final Material material = m.mesh.material;
      ...
      final Int32List renderColors = renderMesh.colors;
      final List<Color> colors = m.mesh.colors;
      final int colorCount = m.mesh.vertices.length;
      if (colorCount != m.mesh.colors.length) {
        final int colorValue = (m.mesh.texture != null)
            ? Color.fromARGB(0, 0, 0, 0).value
            : toColor(m.mesh.material.diffuse, m.mesh.material.opacity).value;
    ...
    final int texcoordCount = m.mesh.vertices.length;
    final Float32List renderTexcoords = renderMesh.texcoords;
    if (m.mesh.texture != null && m.mesh.texcoords.length == texcoordCount) {
      final int imageWidth = m.mesh.textureRect.width.toInt();
      final int imageHeight = m.mesh.textureRect.height.toInt();
      final double imageLeft = m.mesh.textureRect.left;
      final double imageTop = m.mesh.textureRect.top;
      final List<Offset> texcoords = m.mesh.texcoords;
    ...
    // render children
    List<Model> children = m.children;
    for (int i = 0; i < children.length; i++) {
      _renderModel(renderMesh, children[i], model, view, projection);
    }
  }
  ...
  void render(Canvas canvas, Size size) {
    ...
    // create render mesh from modelss
    final renderMesh = _makeRenderMesh();
    _renderModel(renderMesh, world, Matrix4.identity(), camera.lookAtMatrix, camera.projectionMatrix);
  ...
  void modelCreated(Model model) {
    updateTexture();
    if (_onModelCreated != null) _onModelCreated!(model);
  }
  ...
  void _getAllMesh(List<Mesh> meshes, Model model) {
    meshes.add(model.mesh);
    final List<Model> children = model.children;
    for (int i = 0; i < children.length; i++) {
